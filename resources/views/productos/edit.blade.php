...
@section('content')
    <h2 class="text-center mb-2">Editar Producto: {{$producto->titulo}}</h2>
        ...
        <div class="col-md-8">
            <form method="POST" action="{{route('productos.update', ['producto' => $producto->id])}}" enctype="multipart/form-data" novalidate>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="titulo" class="form-control @error('titulo') is-invalid @enderror" id="titulo" placeholder="Título producto" value="{{$producto->titulo}}" />
                    @error('titulo')
                ...
                <div class="form-group mt-3">
                    ...
                    <input type="text" name="descripcion" class="form-control @error('descripcion') is-invalid @enderror" id="descripcion" placeholder="Descripción producto" value="{{$producto->descripcion}}" />
                    @error('descripcion')
                        <span class="invalid-feedback d-block" role="alert">
                            <strong>{{$message}}</strong>
                        </span>
                ...
                <div class="form-group mt-3">
                    ...
                    <input type="number" name="precio" class="form-control @error('precio') is-invalid @enderror" id="precio" placeholder="Precio producto" value="{{$producto->precio}}" />
                    @error('precio')
                        <span class="invalid-feedback d-block" role="alert">
                            <strong>{{$message}}</strong>
                        </span>
